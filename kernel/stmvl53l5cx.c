...
static int stmvl53l5cx_parse_dt(struct device *dev, struct stmvl53l5cx_drvdata *drvdata)
{
	...
	/* Set output to 0 i.e. enable low power mode */
	if (gpiod_direction_output(gpio_pwr, 0) != 0) {
		ret = PTR_ERR(gpio_pwr);
		dev_info(dev, "failed to set output power GPIO: %d.\n", ret);
	}
	drvdata->pwr = gpio_pwr;
...
{
	int ret = 0;
	uint8_t page = 0, revision_id = 0, device_id = 0, i2c_addr;
	char devname[16];

	/* Disable low power mode */
	if (gpiod_direction_output(drvdata->pwr, 1) != 0) {
		pr_err("failed to set output power GPIO: %d.\n", ret);
	}
	mdelay(100);	// Wait for lp mode to disable

	/* Temporary addr before config */
	i2c_addr = drvdata->client->addr;
	drvdata->client->addr = 0x29;

	/* Change device address from 0x29 to appropriate address */
	ret = stmvl53l5cx_write_regs(drvdata, 0x7FFF, &page, 1, NULL);
	ret |= stmvl53l5cx_write_regs(drvdata, 0x4, &i2c_addr, 1, NULL);
	drvdata->client->addr = i2c_addr;
	page = 2;
	ret |= stmvl53l5cx_write_regs(drvdata, 0x7FFF, &page, 1, NULL);
	page = 0;

	if (ret) {
		pr_debug("Write error? ret = 0x%d", ret);
	}
	ret = 0;
	/* Detect device on given address */
	...
	if ((device_id != 0xF0) || (revision_id != 0x02)) {
		...
		pr_info("device_id: 0x%x. revision_id: 0x%x\n", device_id, revision_id);
		return -ENODEV;
	}
